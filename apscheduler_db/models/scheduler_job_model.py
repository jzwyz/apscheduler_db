...
class SchedulerJob(SQLModel, table=True):
    ...
    func_id: str = Field(sa_column=Column(String(100),nullable=False,unique=True,comment="任务ID，唯一"))
    func: str = Field(sa_column=Column(String(100),nullable=False,comment="调度任务执行的函数: app.tasks.request_amz#send_request_amz"))
    name: str = Field(sa_column=Column(String(255),nullable=False,comment="任务名称"))
    trigger: str = Field(default="date",sa_column=Column(String(20),nullable=False,server_default=text("'date'"),comment="任务触发方式: date/cron/interval"))
    ...
    user_id: Optional[int] = Field(default=None,sa_column=Column(Integer,nullable=True,comment="用户ID"))
    unique_key: Optional[str] = Field(default=None,sa_column=Column(String(100),nullable=True,comment="计算任务参数的唯一标识"))
    log_storage_days: int = Field(default=7,sa_column=Column(SMALLINT,nullable=False,server_default=text("7"),comment="保留日志周期，默认7天，最长30天"))
    created_at: datetime = Field(sa_column=Column(TIMESTAMP,nullable=False,server_default=sql_func.current_timestamp(),comment="创建时间"))
    updated_at: datetime = Field(sa_column=Column(TIMESTAMP,nullable=False,server_default=sql_func.current_timestamp(),server_onupdate=sql_func.current_timestamp(),comment="更新时间"))

    # 计算唯一key的函数
    @property
    def calculate_unique_key(self) -> str:
        # 构建字典, 过滤掉对于调度任务执行无影响的字段
        request_dict = self.model_dump(exclude=['created_at','updated_at','user_id','id','log_storage_days','unique_key'])
